...
Server is another text input, but refresh interval could be a `Spinner` with human-readable values such as: Five minutes, Every hour, Once a day. The values would be in seconds though.

LAB: Use static fragments::
Create a main activity that would host both preference fragment as well as status fragment.

LECTURE: Action Bar::
What Action Bar is and how to use it.

DEMO: Create the action bar::
Create an action bar to switch from status activity to preference activity.
